package de.roboticbrain.marmor;

/**
 * Modular Armor Mod
 * 
 * Modular Armor Constants
 * 
 * @author Robotic-Brain
 * @license GNU Public License v3 (http://www.gnu.org/licenses/)
 *
 */
public class Constants {
    public static final String MOD_ID = "ModularArmorMod";
    public static final String VERSION_STRING = "0.0.1";
    
    public static final String CHANNEL_NAME = MOD_ID;
    
    public static final String CLIENT_PROXY_CLASS = "de.roboticbrain.marmor.core.proxy.ClientProxy";
    public static final String SERVER_PROXY_CLASS = "de.roboticbrain.marmor.core.proxy.CommonProxy";
}
